...
plot(data[, 1], data[, 2], pch = 20, type = "l", xaxt = "n", ylim=c(-20,20),
...
sim = iar_sim(100,phi=-0.7)
mean(sim$x)
pars = iar_mle(par = c(-0.99, 0.99),
                        fn = iar_loglik,
                        data = sim,
                        hessian = TRUE)
plot(sim[, 2], sim[, 1], pch = 20, type = "l", xaxt = "n",
           xlab = expression(t[n]), ylab = expression(X[t[n]]), col='gray')
axis(3, at = sim[, 2], col = "red", labels = FALSE)
axis(1, at = seq(0, (max(sim[, 2]) + 50), 50), col = "black")



pars
pred <- calc_xhat(sim, pars$par[1])
lines(sim$t_n, pred, col="red", lty = 1,lwd=1)
